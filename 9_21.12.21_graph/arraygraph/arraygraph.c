...
static void free_all(ArrayGraph **arraygraph, int i)
{
    for (int j = 0; j < i; j++)
    {
...
ArrayGraph* createArrayGraph(int maxVertexCount)
{
    ...
    if (!arraygraph->pVertex)
    {
        ...
        free_all(&arraygraph, maxVertexCount);
        return (NULL);
    }
	return (arraygraph);
}
...
ArrayGraph* createArrayDirectedGraph(int maxVertexCount)
{
    ...
    {
        printf("[error] malloc failure : pVertex\n");
        free_all(&arraydgraph, maxVertexCount);
        return (NULL);
    }
    return (arraydgraph);
}
...
int addEdgeAG(ArrayGraph* pGraph, int fromVertexID, int toVertexID)
{
    ...
    if (!checkVertexValid(pGraph, fromVertexID) || !checkVertexValid(pGraph, toVertexID))
        return (FALSE);
    if (pGraph->ppAdjEdge[fromVertexID][toVertexID] == NOT_USED)
    {
